...
function Storage() {
	...
	return {
		...
		get: function ( callback,firebasedb , project ) {
			
			var docRef = firebasedb.collection("projects").doc(project);
			var MyData;
			docRef.get().then(function(doc) {
				if (doc.exists) {
					MyData = doc.data().data;
				}
				var transaction = database.transaction( [ 'states' ], 'readwrite' );
				var objectStore = transaction.objectStore( 'states' );
				...
				request.onsuccess = function ( event ) {
					...
					callback( event.target.result, MyData );
		...
		set: function ( data, firebasedb, project ) {
			...
			var objectStore = transaction.objectStore( 'states' );
			var request = objectStore.put( data, 0 );
			var file = firebasedb.collection("projects").doc(project);
